...
def main():
    try:
        ...
        # Append to log file
        log_file = Path.home() / '.claude.local' / 'bash-command-log.txt'
        try:
        ...
        daily_log = Path.home() / '.claude.local' / 'hooks' / f"commands-{datetime.now().strftime('%Y-%m-%d')}.log"
        try:
        ...
        stats_file = Path.home() / '.claude.local' / 'hooks' / 'command-stats.json'
        try:
            stats = {}
