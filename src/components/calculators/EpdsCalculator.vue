<template>
  <!--
    What: EPDS calculator orchestrator — the main component mounted by epds.ts.
    How: Uses the useCalculatorForm composable for shared state, renders the
    interactive form and print view side by side (print view hidden on screen,
    visible during @media print).
  -->
  <div class="medical-calculator-container">
    <div class="calculator-print-wrapper">

      <!-- Interactive view — visible on screen, hidden during print -->
      <div class="calculator-interactive-view w-full max-w-[800px] mx-auto px-4">

        <SurfaceCard title="Patient">
          <template #content>
            <PersonInfo
              :name="patient.name"
              :age="patient.age"
              :minAge="config.minAge"
              :maxAge="config.maxAge"
              :gender="patient.gender"
              :showCpr="config.showCpr"
              :cpr="patient.cpr"
              genderdisplay="block"
              @update:name="patient.name = $event"
              @update:age="patient.age = $event"
              @update:gender="patient.gender = $event"
              @update:cpr="patient.cpr = $event"
            />
          </template>
        </SurfaceCard>

        <SurfaceCard :title="config.name">
          <template #content>
            <form @submit.prevent="handleSubmit">
              <QuestionSingleComponent
                name="section1"
                v-for="(question, index) in questions"
                :key="index"
                :question="question"
                :options="question.options"
                :index="index"
                :is-unanswered="formSubmitted && isUnanswered(question)"
                scrollHeight="18rem"
              />

              <div v-if="validationMessage" class="text-red-500 mt-5 font-bold">
                {{ validationMessage }}
              </div>

              <div class="flex justify-end text-right mt-5 gap-3">
                <CopyDialog
                  title="Kopier til Clipboard"
                  icon="pi pi-clipboard"
                  severity="secondary"
                  class="mr-3"
                  :disabled="!hasResults"
                >
                  <template #container>
                    <b>{{ config.name }}</b>
                    <br /><br />
                    Navn: {{ patient.name }} <br />
                    Køn: {{ patient.gender }} <br />
                    Alder: {{ patient.age }} år<br /><br />
                    <template v-if="result">
                      <div v-for="qr in result.questionResults" :key="qr.questionNumber">
                        {{ qr.questionText }} {{ qr.score }}
                      </div>
                      <br /><br />
                      {{ config.name }} {{ result.score }} : {{ result.interpretation }}
                    </template>
                  </template>
                </CopyDialog>

                <SecondaryButton
                  label="Print"
                  icon="pi pi-print"
                  severity="secondary"
                  :disabled="!hasResults"
                  @click="handlePrint"
                />

                <SecondaryButton
                  label="Reset"
                  icon="pi pi-sync"
                  severity="secondary"
                  @click="reset"
                />

                <Button
                  type="submit"
                  label="Beregn"
                  class="pr-6 pl-6 rounded-lg"
                  icon="pi pi-calculator"
                />
              </div>
            </form>
          </template>
        </SurfaceCard>

        <div v-if="hasResults" class="results" ref="resultsSection">
          <SurfaceCard title="Resultat">
            <template #content>
              <br />
              <Message
                class="flex justify-center p-3 text-center"
                :severity="resultSeverityDisplay"
              >
                <h2>{{ config.name }} {{ result!.score }} <br /> {{ result!.interpretation }}</h2>
              </Message>
              <br />
              <p class="text-sm text-center">
                Score ≤ 9: Ikke tegn til alvorlig depression
                Score ≥ 10: Behandlingskrævende depression kan foreligge
              </p>
            </template>
          </SurfaceCard>
        </div>
      </div>

      <!-- Print view — hidden on screen, visible during @media print -->
      <EpdsCalculatorPrint
        :config="config"
        :patient="patient"
        :result="result"
      />

    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, computed } from 'vue'

import Button from '@/volt/Button.vue'
import SecondaryButton from '@/volt/SecondaryButton.vue'
import Message from '@/volt/Message.vue'
import QuestionSingleComponent from '../QuestionSingleComponent.vue'
import CopyDialog from '../CopyDialog.vue'
import SurfaceCard from '../SurfaceCard.vue'
import PersonInfo from '../PersonInfo.vue'
import EpdsCalculatorPrint from './EpdsCalculatorPrint.vue'

import { useCalculatorForm } from '../../composables/useCalculatorForm'
import { epdsConfig, calculateEpds } from '../../scoring/epds'
import sendDataToServer from '../../assets/sendDataToServer'

const config = epdsConfig
const {
  questions,
  patient,
  result,
  formSubmitted,
  validationMessage,
  hasResults,
  validate,
  calculate,
  reset,
  isUnanswered
} = useCalculatorForm(config, calculateEpds)

const resultsSection = ref<HTMLDivElement | null>(null)

const apiUrlServer = import.meta.env.VITE_API_URL
const apiUrl = apiUrlServer + '/index.php/callback/LogCB/log'
const keyUrl = apiUrlServer + '/index.php/KeyServer/getPublicKey'

/**
 * What: Maps ScoreResult severity to PrimeVue Message severity.
 * How: 'severe' → 'error', 'normal' → 'success'.
 */
const resultSeverityDisplay = computed(() => {
  if (!result.value) return 'info'
  return result.value.severity === 'severe' ? 'error' : 'success'
})

// Why: Scoring and transport are separated so the print view can
// read scored state without triggering data submission.
function handleSubmit(): void {
  formSubmitted.value = true

  if (validate()) {
    calculate()
    scrollToResults()
    submitData()
  }
}

function submitData(): void {
  const payload = {
    name: patient.value.name,
    age: patient.value.age,
    gender: patient.value.gender,
    answers: questions.value,
    scores: {
      totalScore: result.value?.score ?? 0
    }
  }

  sendDataToServer(apiUrl, keyUrl, payload)
    .then(() => {})
    .catch(() => {})
}

function handlePrint(): void {
  window.print()
}

function scrollToResults(): void {
  if (resultsSection.value) {
    resultsSection.value.scrollIntoView({ behavior: 'smooth' })
  }
}
</script>
