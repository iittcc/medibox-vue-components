...
import WellsPeCalculatorPrint from './WellsPeCalculatorPrint.vue'

import { useCalculatorForm } from '../../composables/useCalculatorForm'
import { wellsPeConfig, calculateWellsPe, WELLS_PE_SECTIONS, WELLS_PE_RECOMMENDATION } from '../../scoring/wellsPe'
import sendDataToServer from '../../assets/sendDataToServer'

const config = wellsPeConfig
const {
  questions,
  patient,
  result,
  ...
  calculate,
  reset
} = useCalculatorForm(config, calculateWellsPe)
...
function sectionAtIndex(index: number): string | null {
  const section = WELLS_PE_SECTIONS.find(s => s.startIndex === index)
...
const recommendation = computed(() => {
  ...
  return WELLS_PE_RECOMMENDATION[result.value.severity] ?? ''
})

const riskRateTable = [
  { score: '< 2', probability: 'Lav', recommendation: 'D-dimer ved mistanke' },
  { score: '2-5', probability: 'Moderat', recommendation: 'D-dimer nødvendig' },
  { score: '≥ 6', probability: 'Høj', recommendation: 'Billeddiagnostik direkte' }
]
