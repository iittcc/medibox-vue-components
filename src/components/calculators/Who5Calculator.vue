          ...
          <SurfaceCard title="Resultat">
            <template #content>
              ...
              <p class="text-sm text-center">
                Score ≤ 35: Stor risiko for depression eller stressbelastning
                Score 36-50: Risiko for depression eller stressbelastning
                Score &gt; 50: Ingen risiko for depression eller stressbelastning
              </p>
            </template>
          </SurfaceCard>
        </div>
...
import Who5CalculatorPrint from './Who5CalculatorPrint.vue'

import { useCalculatorForm } from '../../composables/useCalculatorForm'
import { who5Config, calculateWho5 } from '../../scoring/who5'
import sendDataToServer from '../../assets/sendDataToServer'

const config = who5Config
const {
  questions, patient, result, formSubmitted, validationMessage,
  hasResults, validate, calculate, reset, isUnanswered
} = useCalculatorForm(config, calculateWho5)
...
function handleSubmit(): void {
  ...
  if (validate()) {
    ...
    sendDataToServer(apiUrl, keyUrl, {
      name: patient.value.name, age: patient.value.age,
      gender: patient.value.gender, answers: questions.value,
      scores: { totalScore: result.value?.score ?? 0 }
    }).then(() => {}).catch(() => {})
  }
}
