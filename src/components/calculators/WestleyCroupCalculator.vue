                ...
                <CopyDialog title="Kopier til Clipboard" icon="pi pi-clipboard" severity="secondary" class="mr-3" :disabled="!hasResults">
                  <template #container>
                    ...
                    <template v-if="result">
                      ...
                      <br /><br />{{ config.name }} {{ result.score }} : {{ result.interpretation }}
                    </template>
                  </template>
                </CopyDialog>
        ...
        <div v-if="hasResults" class="results" ref="resultsSection">
          <SurfaceCard title="Resultat">
            <template #content>
              ...
              <p class="text-sm text-center">Score ≤ 2: Mild | Score 3-7: Moderat | Score ≥ 8: Svær pseudocroup</p>
            </template>
          </SurfaceCard>
        </div>
      </div>

      <WestleyCroupCalculatorPrint :config="config" :patient="patient" :result="result" />
    </div>
  </div>
</template>
...
import WestleyCroupCalculatorPrint from './WestleyCroupCalculatorPrint.vue'
import { useCalculatorForm } from '../../composables/useCalculatorForm'
import { westleyCroupConfig, calculateWestleyCroup } from '../../scoring/westleyCroupScore'
import sendDataToServer from '../../assets/sendDataToServer'

const config = westleyCroupConfig
const { questions, patient, result, formSubmitted, validationMessage, hasResults, validate, calculate, reset, isUnanswered } = useCalculatorForm(config, calculateWestleyCroup)
