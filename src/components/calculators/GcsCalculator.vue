<template>
  <div class="medical-calculator-container">
    <div class="calculator-print-wrapper">
                ...
                <CopyDialog title="Kopier til Clipboard" icon="pi pi-clipboard" severity="secondary" class="mr-3" :disabled="!hasResults">
                  <template #container>
                    ...
                    <template v-if="result">
                      ...
                      <br /><br />{{ config.name }} Score {{ result.score }} : {{ result.interpretation }}
                    </template>
                  </template>
                </CopyDialog>
          ...
          <SurfaceCard title="Resultat">
            <template #content>
              ...
              <Message class="flex justify-center p-3 text-center" :severity="resultSeverityDisplay">
                <h2>{{ config.name }} Score {{ result!.score }} : {{ result!.interpretation }}</h2>
              </Message><br />
            </template>
          </SurfaceCard>
        </div>
      ...
      <GcsCalculatorPrint :config="config" :patient="patient" :result="result" />
    </div>
  </div>
</template>
...
import GcsCalculatorPrint from './GcsCalculatorPrint.vue'
import { useCalculatorForm } from '../../composables/useCalculatorForm'
import { gcsConfig, calculateGcs } from '../../scoring/gcs'
import sendDataToServer from '../../assets/sendDataToServer'

const config = gcsConfig
const { questions, patient, result, formSubmitted, validationMessage, hasResults, validate, calculate, reset, isUnanswered } = useCalculatorForm(config, calculateGcs)
...
const resultSeverityDisplay = computed(() => {
  ...
  if (result.value.severity === 'moderate') return 'error'
  if (result.value.severity === 'mild') return 'warn'
  return 'success'
})
