        ...
        <div v-if="hasResults" class="results" ref="resultsSection">
          <SurfaceCard title="Resultat">
            <template #content>
              ...
              <p class="text-sm text-center">Score ≤ 6: Mild | Score 7-12: Moderat | Score ≥ 13: Svær (Hyperemesis Gravidarum)</p>
            </template>
          </SurfaceCard>
        </div>
      </div>

      <PuqeCalculatorPrint :config="config" :patient="patient" :result="result" />
    </div>
  </div>
</template>
...
import PuqeCalculatorPrint from './PuqeCalculatorPrint.vue'
import { useCalculatorForm } from '../../composables/useCalculatorForm'
import { puqeConfig, calculatePuqe } from '../../scoring/puqe'
import sendDataToServer from '../../assets/sendDataToServer'

const config = puqeConfig
const { questions, patient, result, formSubmitted, validationMessage, hasResults, validate, calculate, reset, isUnanswered } = useCalculatorForm(config, calculatePuqe)
