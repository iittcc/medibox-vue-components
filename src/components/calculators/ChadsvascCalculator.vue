          ...
          <template #content>
            <div class="mb-4 pl-1">
              ...
              <p class="text-sm text-gray-500 mt-1">
                Beregning af indikation for AK-behandling ved non-valvulær atrieflimren/flagren
              </p>
            </div>
          ...
          <SurfaceCard title="Resultat">
            <template #content>
              ...
              >
                ...
                <p class="text-sm mt-1">Tromboembolirate: {{ annualRisk }} pr. år</p>
              </Message>
              <br />
                ...
                <table class="w-full text-sm">
                  <thead>
                    <tr class="border-b border-gray-200" :style="{ backgroundColor: 'var(--p-primary-50)' }">
                      ...
                      <th class="px-3 py-2 text-left font-semibold text-gray-700">Tromboembolirate</th>
                  ...
                  <tbody>
                    ...
                    >
                      ...
                      <td class="px-3 py-1.5">{{ row.rate }} pr. år</td>
...
import ChadsvascCalculatorPrint from './ChadsvascCalculatorPrint.vue'

import { useCalculatorForm } from '../../composables/useCalculatorForm'
import { chadsvascConfig, calculateChadsvasc, CHADSVASC_ANNUAL_RISK, CHADSVASC_ANNUAL_RISK_GTE2 } from '../../scoring/chadsvasc'
import sendDataToServer from '../../assets/sendDataToServer'

const config = chadsvascConfig
const {
  questions,
  patient,
  result,
...
watch(() => questions.value[AGE_QUESTION_INDEX].answer, (answer) => {
  ...
  if (answer === 0 && age >= 65) patient.value.age = 64
  else if (answer === 1 && (age < 65 || age >= 75)) patient.value.age = 65
  else if (answer === 2 && age < 75) patient.value.age = 75
})
...
const factors = computed(() =>
  questions.value.map(q => {
    ...
    const maxPoints = Math.max(...q.options.map(o => o.value))
    return {
      letter: parts[0],
      name: parts[1] || q.text,
      pointLabel: q.options.length > 2 ? `0–${maxPoints}` : `+${maxPoints}`
    }
  })
)
...
const annualRisk = computed(() => {
  ...
  return CHADSVASC_ANNUAL_RISK[result.value.score] ?? CHADSVASC_ANNUAL_RISK_GTE2
})

const riskRateTable = [
  { score: '0', rate: '0%', recommendation: 'Ingen behandling' },
  { score: '1', rate: '1,3%', recommendation: 'Bør overvejes' },
  { score: '≥2', rate: '≥2,2%', recommendation: 'AK-behandling anbefales' }
]
